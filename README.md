...
3. Deploy, then generate a domain the same way. The web Docker image renders its Nginx config from Railway env at startup and listens on Railway's runtime `PORT` (local Docker defaults to 80), so you do not need a custom port setting unless you deliberately override it.
